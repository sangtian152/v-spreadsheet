...
function editorSet() {
  ...
  // console.log(data.getSelectedCell())
...
function selectMenuChange(value, txt){
  dataSetCellText.call(this, txt, 'input');
  editorSet.call(this);
}
function sheetInitEvents() {
  ...
  overlayerEl
    ...
    .on('mousedown', (evt) => {
      ...
      } else if (evt.detail === 2) { // 单元格鼠标双击
        console.log('dblclick')
        const { data } = this;
        const cell = data.getSelectedCell()
        if(cell && cell.source){
          const cellRect = data.getCellRectByXY(evt.offsetX, evt.offsetY);
          const {
            left, top, width, height,
          } = cellRect;
          selectMenu.set(cell.source)
          selectMenu.setPosition(left, top, height);
        }
        editorSet.call(this);
      } else {
        editor.clear();
  ...
  // selectMenu ok 
  selectMenu.ok = (value, txt) => selectMenuChange.call(this, value, txt)
...
export default class Sheet {
  constructor(targetEl, data) {
    ...
    // selectMenu
    this.selectMenu = new SelectMenu(() => this.getTableOffset(), !showContextmenu);
    // selector
